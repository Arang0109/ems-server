...
> 적용 버전: jxls-poi 3.1 / 문서 개정일 2026-09-22 (커스텀 필드 `custom`·템플릿 검사 추가)

> ⚠️ **개정 필요(운영 메모).** 이 문서의 일부는 현재 서버와 어긋나 있습니다 — "성적서 발행(단일 xlsx)" 내보내기는 현재 없고
> 채취기록부 ZIP만 있습니다. ZIP 안 파일명 규칙도 다릅니다. §4·§5의 변수명 일부(`plan.mentor`→`plan.mentorName`,
> `plan.clientAddress`→`plan.clientRoadAddress`+`plan.clientDetailAddress`, `sheet.pointCount`→`sheet.samplingPointCount`,
> `moisture.ratio`→`moisture.xw`, `flow.velocity`→`flow.avgVs` 등)도 실제 이름과 다릅니다. **정확한 이름은 템플릿 검사
> (§8-0)가 알려 줍니다.** 전면 개정은 별건으로 진행합니다.
...
| `custom` | **고객사가 직접 정의한 커스텀 필드** (§4-10) | `plan.customFields` |

```
${plan.workplaceName} / ${sheet.category} / ${moisture.xw} / ${flow.standardQuantity} / ${custom.siteCode}
```
...
시스템이 제공하는 이름(§4·§5) 밖의 칸 — 현장 코드, 결재선, 비고처럼 **고객사마다 다른 값** — 을 성적서에 넣을 때 씁니다.

1. **관리자가 이름을 정의합니다.** 시스템 설정 > 커스텀 필드에서 `키`와 `라벨`을 등록합니다.
   키는 템플릿에서 쓰는 이름이라 **영문자·숫자·밑줄만**, 영문자 또는 밑줄로 시작해야 하며(예: `siteCode`, `approver_1`),
   **한 번 등록한 키는 바꿀 수 없습니다**(배포된 양식이 그 이름을 참조하기 때문). 이름을 바꾸려면 삭제 후 다시 등록합니다.
2. **회차마다 값을 채웁니다.** 측정계획 화면의 커스텀 필드 탭에 정의된 칸이 나타나고, 거기에 적은 값이 그 회차의 성적서에 찍힙니다.
3. **템플릿에서 읽습니다.**

```
${custom.siteCode}          ← 점 표기
${custom['siteCode']}       ← 대괄호 표기. 둘은 같습니다
현장: ${custom.siteCode} / 결재: ${custom.approver_1}
```

| 상황 | 결과 |
|------|------|
| 정의되어 있고 값을 채운 키 | 그 값 |
| 정의는 있지만 이 회차에 값을 안 채운 키 | **빈칸** |
| 정의에 없는 키(오타 포함) | **빈칸** — 오류가 나지 않으니 §8-0 템플릿 검사로 잡습니다 |

> 커스텀 필드의 값은 항상 **문자**입니다. 숫자로 계산해야 하면 셀 서식이 아니라 `$[VALUE(...)]` 수식으로 감싸 주세요.
> 정의를 삭제해도 이미 저장된 회차의 값은 남아 있어, 양식이 그 키를 계속 참조하면 계속 찍힙니다.
...
**없는 이름(오타)도 빈칸입니다.** 내보내기가 실패하지 않으므로 오타를 결과물에서 알아채기 어렵습니다 —
양식을 등록하기 전에 §8-0 **템플릿 검사**를 꼭 거치세요. 유일한 예외는 `jx:area` 영역 **밖**의 셀입니다.
그 셀은 평가되지 않아 `${...}` 글자가 그대로 남습니다.
...
양식을 등록하기 전에 **템플릿 검사**를 돌리세요(API `POST /api/schedules/sampling-records/template-check`, 양식 파일을 `template` 파트로 업로드. 화면 연동은 프론트엔드 작업 후 안내).
양식을 실제로 채우지 않고 `${...}`와 `jx:` 메모를 읽어, 시스템이 모르는 이름을 **시트·셀 주소와 함께** 알려 줍니다.

| 종류 | 뜻 | 예 |
|------|----|----|
| `UNKNOWN_ROOT` | 최상위 변수(§3)가 아닌 이름 | `${pointz[0].ts}` |
| `UNKNOWN_PROPERTY` | 변수는 맞는데 그 아래 이름이 없음. 틀린 지점까지의 경로를 보여 줍니다 | `${plan.clientNmae}` |
| `UNKNOWN_CUSTOM_KEY` | `custom` 아래 키가 우리 회사 커스텀 필드 정의에 없음 | `${custom.siteCod}` |
| `PARSE_ERROR` | 표현식 문법 오류. **이것만 내보내기가 실패**합니다 | `${plan.name +}` |
| `AREA_MISSING` | 시트에 `${...}`가 있는데 `jx:area` 메모가 없음 — 그 시트는 채워지지 않습니다 | A1 메모 누락 |

`jx:each`의 반복 변수(`var="p"`)도 `items`가 가리키는 목록의 항목 기준으로 검사합니다(`${p.tz}` → `UNKNOWN_PROPERTY`).
결과가 `valid`이면 이름 오류는 없습니다(값이 비어 있어 빈칸이 되는 것은 검사 대상이 아닙니다).
...
2. **표현식 문법 오류** — 변수명 오타는 실패가 아니라 빈칸이 됩니다(§7). 실패한다면 `${...}` 안의 문법(짝이 안 맞는 괄호·연산자)을 보세요. §8-0 검사가 `PARSE_ERROR`로 짚어 줍니다.
