...
db.schedule_documents.find({ "items.0": { $exists: true } }).forEach(doc => {
  ...
  for (const r of records) {
    ...
    byPollutant.set(r.pollutantId, r);
  }
  ...
  const items = doc.items.map(item => {
    ...
    const r = byPollutant.get(item.pollutantId);
    if (!r) return item;

    matched.add(item.pollutantId);
    grafted++;
  ...
  for (const [pollutantId, r] of byPollutant) {
    if (!matched.has(pollutantId)) {
