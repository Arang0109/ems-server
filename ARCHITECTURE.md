...
├── chat/                      # 사용자 간 1:1 대화 (MySQL + MongoDB + Redis, STOMP)
...
| MongoDB | `chat` | `chat_messages`(대화 본문 + 첨부 메타) — 대량 append-only 이고 첨부처럼 뒤에 붙을 구조가 유동적임 |
| MongoDB | `schedule` | `schedule_documents`(측정 시점 스냅샷 + 측정 시트 + 실험분석정보) |
| Redis | `auth` | Refresh Token |
| Redis | `chat` | 접속 상태(`PRESENCE:*`) — 원장이 아니라 TTL 로 스스로 만료되는 휘발성 상태 |
...
"되돌릴 수 없는 쪽을 마지막에"는 결론이지 원리가 아닙니다. 모듈마다 순서가 달라지므로 기준을 적어 둡니다.

1. **실패했을 때 남는 상태 중 덜 위험한 것을 고른다.** 고아 파일은 아무에게도 보이지 않지만,
   레코드가 가리키는 파일이 없으면 사용자가 깨진 화면을 봅니다.
2. **참조는 참조 대상보다 뒤에 온다.** 식별자를 부여하는 쓰기가 먼저여야 그것을 가리키는 쓰기가 가능합니다.

| 모듈 | 순서 | 기준 |
|---|---|---|
| `storage` | 메타(MySQL) → 파일 | 파일 쓰기가 실패하면 트랜잭션이 롤백되어 실물 없는 레코드가 남지 않습니다(기준 1) |
| `schedule` | 메타(MySQL) → 문서(Mongo) | 메타가 진실의 원천이고 문서 저장이 트랜잭션의 마지막 부수효과입니다(기준 1) |
| `chat` | MySQL → 파일 → Mongo → MySQL 요약 | 첨부 메타만 남는 쪽이 더 위험해 파일이 문서보다 앞이고(기준 1), 방의 요약은 Mongo 가 부여한 id 를 참조하므로 문서보다 뒤입니다(기준 2) |

`chat` 의 마지막 단계는 롤백 가능하므로 실패해도 "요약이 갱신되지 않은 채 메시지만 남는" 상태로
끝납니다 — 그 메시지는 방을 열면 정상적으로 보이고 다음 전송이 요약을 바로잡습니다.
**MySQL 이 존재하지 않는 문서를 가리키는 일은 세 모듈 어디에서도 일어나지 않습니다.**
