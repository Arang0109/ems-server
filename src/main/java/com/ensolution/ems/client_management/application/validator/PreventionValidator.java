...
/**
 * 방지시설(Prevention)의 비즈니스 규칙 검증을 담당한다.
 */
@Component
public class PreventionValidator {

	/**
	 * 순서 변경 요청이 이 측정지점의 방지시설 집합과 정확히 일치하는지 확인한다.
	 * <p>
	 ...
	 */
	public void requireExactOrder(List<Prevention> current, List<Long> orderedIds) {
		if (Set.copyOf(orderedIds).size() != orderedIds.size()) {
			throw new CustomException(ErrorCode.PREVENTION_ORDER_MISMATCH);
		}

		Set<Long> currentIds = current.stream()
			.map(Prevention::getId)
