...
public interface ChatMessageMapper {
	...
	/**
	 * 첨부 전송. 표현이 둘(JSON·multipart)일 뿐 유스케이스는 하나이므로 같은 Command 로 모은다.
	 * 본문은 캡션이라 없어도 된다.
	 */
	@Mapping(target = "content", source = "request.content")
	@Mapping(target = "clientMessageId", source = "request.clientMessageId")
	@Mapping(target = "attachment", source = "attachment")
	SendMessageCommand toSendCommand(SendAttachmentRequest request, AttachmentUpload attachment,
