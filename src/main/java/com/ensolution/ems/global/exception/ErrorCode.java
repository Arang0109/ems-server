...
public enum ErrorCode {
  ...
  // 도메인 특화: {DOMAIN}_{CONDITION} 패턴
	SCHEDULE_INVALID_CHRONOLOGY(HttpStatus.BAD_REQUEST, "보고서 진행 일자의 순서가 올바르지 않습니다"),
  ...
  SCHEDULE_SHEET_NOT_EDITABLE(HttpStatus.CONFLICT, "분석값 입력이 완료된 측정계획의 채취 기록지는 수정할 수 없습니다."),
  ...
  SCHEDULE_MEASURER_DUPLICATED(HttpStatus.BAD_REQUEST, "사수와 부사수는 같은 사용자일 수 없습니다."),
  CUSTOM_FIELD_NOT_FOUND(HttpStatus.NOT_FOUND, "존재하지 않는 커스텀 필드입니다."),
  CUSTOM_FIELD_KEY_ALREADY_EXISTS(HttpStatus.CONFLICT, "이미 같은 키의 커스텀 필드가 있습니다."),
  CUSTOM_FIELD_INVALID_KEY(HttpStatus.BAD_REQUEST, "커스텀 필드 키는 영문자·숫자·밑줄만 쓸 수 있고 영문자 또는 밑줄로 시작해야 하며, 예약어는 쓸 수 없습니다."),
  CUSTOM_FIELD_NOT_DEFINED(HttpStatus.BAD_REQUEST, "정의되지 않은 커스텀 필드입니다."),
  ...
  MEASUREMENT_METHOD_GROUPING_MISMATCH(HttpStatus.BAD_REQUEST, "통칭 시료명은 한 병으로 함께 채취하는 측정방법에만 지정할 수 있습니다."),
