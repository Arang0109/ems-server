 ...
 * 잔여일수는 대시보드가 기준일(baseDate)을 정해 계산하는 파생값이며, 교정 기한이 지난 장비는 음수가 된다.
 */
@Mapper(componentModel = "spring", imports = ChronoUnit.class)
public interface EquipmentPortMapper {

	@Mapping(target = "daysRemaining",
		expression = "java(ChronoUnit.DAYS.between(baseDate, summary.nextCalibrationDate()))")
	CalibrationDue toCalibrationDue(CalibrationDueSummary summary, @Context LocalDate baseDate);

	List<CalibrationDue> toCalibrationDues(List<CalibrationDueSummary> summaries, @Context LocalDate baseDate);
}
