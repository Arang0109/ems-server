 ...
 *   <li>유량 집계 — 전량 계산 결과다. 옛 회차의 집계를 들고 오면 새 측정점 입력과 어긋난 값이
 *       화면에 남고, 저장 시 재계산되기 전까지 그것이 이번 회차의 유량처럼 보인다</li>
 * </ul>
 */
public final class SheetReuse {
	...
	public static SamplingSheet forNextRound(SamplingSheet source) {
		...
		return source.toBuilder()
			...
			.flowRate(null)
			.particulateSampling(clearTimes(source.getParticulateSampling()))
			.gaseousSamplings(clearSampleIdentity(source.getGaseousSamplings()))
			.build();
	}
	
	private static WeatherData keepPressure(WeatherData weather) {
		if (weather == null || weather.getAtmosphericPressure() == null) return null;
		return WeatherData.builder().atmosphericPressure(weather.getAtmosphericPressure()).build();
	}
	...
	private static ParticulateSampling clearTimes(ParticulateSampling particle) {
	 ...
	 */
	private static List<GaseousSampling> clearSampleIdentity(List<GaseousSampling> gaseousSamplings) {
		if (gaseousSamplings == null) return null;
		return gaseousSamplings.stream()
