...
public class FlowExportView {
	...
	private final BigDecimal avgTs;      // 평균 배출가스 온도 avgTs (℃)
	private final BigDecimal avgTg;     // 평균 배출가스 절대온도 avgTg (K)
	private final BigDecimal avgPv;  // 평균 동압 avgPv (mmH2O)
	private final BigDecimal avgPs;   // 평균 정압 avgPs (mmH2O)
	
	private final BigDecimal gasDensity;             // 현장 배출가스 밀도 (kg/m³)
	private final BigDecimal stackArea;                // 측정시설 단면적 (m²)
	
	private final BigDecimal avgVs;            // 평균 유속 Vs (m/s)
	private final BigDecimal quantity;            // 현장 습윤 유량 (m³/h)
	private final BigDecimal standardQuantity;    // 표준상태 건조 유량 (Sm³/h)
	
	private final BigDecimal cp;    // 피토관 계수 Cp (무차원)
}
