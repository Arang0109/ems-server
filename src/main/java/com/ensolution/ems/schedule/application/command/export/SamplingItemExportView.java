 ...
 * <p>
 * <b>채취시간은 성적서 탭에서 항목별로 작성한 값이며, 서버가 기록지에서 파생하지 않는다.</b>
 * 기록지는 알데히드류를 {@code VOCs}로 통칭해 시료 한 건으로 적지만 성적서는 포름알데히드·
 * 아세트알데히드를 각각 쓴다 — 시료 1건 ↔ 항목 N건이다.
 * <p>
 * 그 대응 자체는 {@code Sample#pollutantIds}에 기록되므로 이제 데이터로 존재한다. 다만
 * <b>통칭 규칙은 여전히 서버가 고정하지 않는다</b> — 어느 병에 무엇을 담을지는 업체와 현장이
 * 정하고, 사용자가 화면에서 행을 쪼개거나 합칠 수 있기 때문이다. 시각을 옮기는 것도 사용자가
 * 버튼을 눌러 확인한 뒤에만 일어난다. 서버가 자동으로 옮기면 실험실에서 고쳐 둔 값이 조용히
 * 되돌아가 틀린 값이 성적서에 찍힌다.
 * <p>
