...
import com.ensolution.ems.schedule.domain.sampling.SheetReuse;
 ...
 *
 * <p>거슬러 올라가는 깊이는 {@value #MAX_LOOK_BACK}회로 제한한다. 세부 스냅샷이 문서 저장소에 있어
...
public class PreviousSheetFinder {
	...
	private static final int MAX_LOOK_BACK = 10;

	private final ScheduleRepository scheduleRepository;
	private final ScheduleDocumentRepository scheduleDocumentRepository;

	/**
	 * 같은 측정시설에서 이 계획보다 앞서 완료된 회차 중, 해당 기록지를 쓴 시트를 반환한다.
	 * 불러올 기록이 없으면 null이다 — 첫 회차이거나 그 기록지를 처음 쓰는 경우이며 오류가 아니다.
	 *
	 * <p>{@code sourceScheduleId}가 null이면 가장 최근 회차를 쓴다.
	 *
	 * <p>지정한 회차가 후보 범위(최근 완료 {@value #MAX_LOOK_BACK}회) 밖이거나 그 사이 재개방되어
	...
	public PreviousSheetDetail find(Schedule current, MeasurementCategory category, Long sourceScheduleId) {
		for (Schedule candidate : lookBack(current)) {
			...
			SamplingSheet sheet = sheetOf(candidate, category);
	...
	private List<Schedule> lookBack(Schedule current) {
		return scheduleRepository.findRecentCompletedBefore(
			current.getStackId(), current.getTenantId(), current.getSampledAt(), MAX_LOOK_BACK);
	}

	private SamplingSheet sheetOf(Schedule schedule, MeasurementCategory category) {
