...
class MeasurementRecordRecorderTest {
	...
	void setUp() {
		...
		recorder = new MeasurementRecordRecorder(repository);
	}
	...
	private static SamplingItemSnapshot withAnalysis(SamplingItemSnapshot item, String value, String unit) {
		return item.withAnalysis(AnalysisResult.empty()
			.applyAnalysisResult(new BigDecimal(value), unit, "주시험법", "분석장비"));
	}
	...
	class ResultValues {
		...
		void copiesAnalysisValue() {
			SamplingItemSnapshot item = withAnalysis(item(1L, 11L, MeasurementCycle.QUARTERLY), "80", "ppm");
		...
		void judgesByAnalysisValueWhenOxygenNotApplicable() {
			SamplingItemSnapshot item = withAnalysis(item(1L, 11L, MeasurementCycle.QUARTERLY, false), "120", "ppm");
		...
		void judgesWithinAllowance() {
			SamplingItemSnapshot item = withAnalysis(item(1L, 11L, MeasurementCycle.QUARTERLY, false), "80", "ppm");
		...
		void holdsJudgementWhenOxygenApplicable() {
			SamplingItemSnapshot item = withAnalysis(item(1L, 11L, MeasurementCycle.QUARTERLY, true), "120", "ppm");
		...
		// "다른 계획의 분석 결과가 섞인다"·"같은 물질의 결과가 둘"·"측정물질을 알 수 없는 결과"는
		// 분석 결과가 측정항목 안으로 들어오면서 구조적으로 성립하지 않게 되어 케이스를 걷어냈다.

		@Test
		@DisplayName("분석 결과가 없는 항목은 판정 근거만 남기고 결과값을 비운다")
		void leavesResultEmptyWithoutAnalysis() {
			...
			assertThat(repository.all().getFirst().result().allowance()).isEqualByComparingTo(ALLOWANCE);
		}
	}
